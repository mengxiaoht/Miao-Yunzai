    ...
    beforeRender({ data }) {
      ...
      if (data.pluginName !== false) {
        ...
        if (data.pluginVersion !== false) {
          pluginName += `<span class="version">${data.pluginVersion || CFG.package?.version || '4.0.0-rc.0'}`
        }
      }
      ...
      return {
        ...
        copyright: `Created By ${CFG.package?.name}<span class="version">${CFG.package?.version}</span>${pluginName}</span>`
      }
    }
  })
}
